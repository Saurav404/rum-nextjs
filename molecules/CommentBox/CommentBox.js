...
export const CommentBox = ({ name, time, msg, data, blogs, slug }) => {
  ...
  const likePost = async () => {
    try {
      await axios.patch(
        `${process.env.NEXT_PUBLIC_STRAPI_API_URL}/comments/${slug}:${blogs.id}/comment/${data.id}/like`
      );
      setLike(like + 1);
    } catch (e) {
      console.error(e);
  ...
  const onSubmit = async (values, { resetForm }) => {
    try {
      await axios.post(`${process.env.NEXT_PUBLIC_STRAPI_API_URL}/comments/${slug}:${blogs.id}`, {
